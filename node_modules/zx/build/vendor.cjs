...
var processInput = (child, input) => {
  if (input && child.stdin && !child.stdin.destroyed) {
    if (input instanceof import_node_stream6.Stream) {
...
var VoidWritable = class extends import_node_stream6.Transform {
  ...
  try {
    if (c.sync) {
      ...
      c.run(() => {
        ...
        (_b2 = child.stdout) == null ? void 0 : _b2.on("data", (d2) => {
          c.store.stdout.push(d2);
          c.store.stdall.push(d2);
          c.ee.emit("stdout", d2, c);
        }).pipe(c.stdout);
        (_c = child.stderr) == null ? void 0 : _c.on("data", (d2) => {
          c.store.stderr.push(d2);
          c.store.stdall.push(d2);
...
var assign2 = (target, ...extras) => Object.defineProperties(target, extras.reduce((m2, extra) => __spreadValues(__spreadValues({}, m2), Object.fromEntries(Object.entries(Object.getOwnPropertyDescriptors(extra)).filter(([, v2]) => !Object.prototype.hasOwnProperty.call(v2, "value") || v2.value !== void 0))), {}));
...
var buildSpawnOpts2 = ({ spawnOpts, stdio, cwd, shell, input, env: env2, detached, signal }) => __spreadProps(__spreadValues({}, spawnOpts), {
  env: env2,
  cwd,
  stdio,
  shell,
...
var isBin = (f3) => {
  if (f3 === "") return false;
  if (!f3.includes("/")) return true;
  if (!import_node_fs6.default.existsSync(f3)) return false;
  const stat = import_node_fs6.default.lstatSync(f3);
  return stat.isFile() || stat.isSymbolicLink();
};
var lookup = (query = {}, cb = noop4) => _lookup({ query, cb, sync: false });
var lookupSync = (query = {}, cb = noop4) => _lookup({ query, cb, sync: true });
...
var _lookup = ({
  ...
  const callback = (err, { stdout }) => {
    ...
    result = parseProcessList(extract2(stdout), query);
    resolve(result);
    cb(null, result);
  };
  const ctx = IS_WIN ? {
